...
Then, for each actionable command-improvement suggestion, file it the same way
the loop files enhancements:

- If an issue-tracker mechanism is available (the issue-tracker MCP server, or a
  configured issue-tracker CLI): create an issue — project
  {{trackerProjectKey}}, type Task, parent {{trackerParentIssue}}, label
  `tooling` — titled `chore(pr-loop): <improvement>`, body linking this report
  and the PR. Record the key.
- Otherwise: write `.pr-loop/enhancements/<slug>.md` (Context / Proposed /
  Acceptance criteria), paste-ready for the tracker.

Return to the caller: the report path, the list of created issue keys / md
paths, and a ≤5-line summary. Do NOT edit the workflow files or the code — you
are report-only; humans (or a later approved step) apply the changes.
